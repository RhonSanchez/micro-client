...
def delete():
    id=request.args["id"]
    if not id:
        logging.warning("QUERY PARAMETER ID DOSEN'T EXIST DELETE")
    ...
    try:
        if use_cases_client.delete_by_id(id):
            ...
            logging.info("ID {%s} DOSEN'T EXIST DELETE", id)
            return {"message": f"client {id} dosen't exist"}, 404
    except Exception as ex:
...
def update():
    ...
    try:
        if use_cases_client.update(Client(**data)):
            ...
            logging.warning("CLIENT {%s} DOSEN'T EXISTS ", data["id"])
            return {"message": "CLIENT DOSEN'T EXISTS1"}, 404
    except Exception as ex:
